...
def create_exp_files_WN():  
    ...
    for category in range(1,9):       
        for coef_number in category_inds[category-1]: #which coefficient to modify
            coef_ind += 1
            if coef_ind in [2,4,6,15,17,19,24,26,28,37,39,41]: #if in coefficients of interest for either WN or non WN...
                #how much to modify it by
                for modification_factor in [round(-1+.1*i,1) for i in range(0,21)]:  #[0:5] +[round(-1+.1*i,1) for i in range(0,21)][16:21]:
     #[round(-.5+.1*i,1) for i in range(0,11)]:
                    #read from base file, write to new outfile
...
'''
def experimental_results_table(outfile_fn='experimental_table_comp_unbounded2.csv',t100ranked_fn = "nonstop_competitive_markets.csv"):
    end_coef = 23
    resTABLE = pd.DataFrame(index=list(range(1,end_coef)),columns=[round(-.5+.1*i,1) for i in range(0,11)])    
    for i in range(1,end_coef):        
        for modification_factor in [round(-.5+.1*j,1) for j in range(0,11)]:#[round(-.5,1),round(-.2,1),round(.2,1),round(.5,1)]:#[round(-.5+.1*j,1) for j in range(0,11)]:
            input_fn = "matlab_2stagegames/exp_results__%s_%s.txt" % (i,modification_factor)
            try:                
                #read in original market profile file    
                t100ranked  = pd.read_csv(t100ranked_fn) 
                #use subset of this as base for results table
                ...
                reduced_net = network_results.set_index('UNIQUE_CARRIER').loc[['WN']] 
...
def weighted_results_table(outfile_fn='experimental_table_comp_unbounded2.csv',t100ranked_fn = "nonstop_competitive_markets.csv"):
    ...
    for airline in [1,2,3,4]:
        resTABLEs[airline-1] = pd.DataFrame(index=[2,4,6,15,17,19,24,26,28,37,39,41],columns=[round(-1+.1*j,1) for j in range(0,21)])    
        for i in [2,4,6,15,17,19,24,26,28,37,39,41]:        
            for modification_factor in [round(-1+.1*j,1) for j in range(0,21)]:#[round(-.5,1),round(-.2,1),round(.2,1),round(.5,1)]:#[round(-.5+.1*j,1) for j in range(0,11)]:
                input_fn = "matlab_2stagegames/exp_results_fixed_WNsep%s_%s_%s.txt" % (airline, i,modification_factor)
                try:                
                    #read in original market profile file    
                    t100ranked  = pd.read_csv(t100ranked_fn) 
                    #use subset of this as base for results table
    ...
    full_restable.to_csv('weighted_MAPE_fixed.csv')
